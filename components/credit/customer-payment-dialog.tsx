...
import type { Customer, PayMethod } from '@/lib/mock-data'
...
export function CustomerPaymentDialog({ open, onClose, customer }: Props) {
  const { payCustomer } = useStore()
  ...
  useEffect(() => {
    if (open && customer) setAmount(customer.balance > 0 ? String(customer.balance.toFixed(2)) : '')
  ...
  const handlePay = async () => {
    ...
    setSaving(true)
    await payCustomer(customer.id, value, note.trim() || null, method)
    toast.success(`გადახდა მიღებულია: ₾${value.toFixed(2)}`)
    setSaving(false)
    onClose()
  }
      ...
      <DialogContent className="max-w-sm rounded-[24px] p-6">
        <DialogHeader>
          <DialogTitle className="text-xl font-black flex items-center gap-2">
            <Wallet className="size-5 text-emerald-500" /> გადახდის მიღება
          </DialogTitle>
          <DialogDescription>
            <span className="font-semibold text-foreground">{customer.name}</span> — მიმდინარე ვალი:{' '}
        ...
        <div className="flex flex-col gap-3.5 py-2">
          ...
          {value > customer.balance && customer.balance >= 0 && (
            <p className="text-[11px] text-amber-600 bg-amber-50 rounded-lg px-3 py-2">
              თანხა ვალზე მეტია — სხვაობა ({(value - customer.balance).toFixed(2)}₾) წინასწარ გადახდად ჩაიწერება.
            </p>
          )}
        </div>
